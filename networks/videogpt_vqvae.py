...
from networks.videogpt.attention import MultiHeadAttention
from networks.videogpt.utils import shift_dim
...
class videogpt_vqvae(nn.Module):
    def __init__(self, config):
        super(videogpt_vqvae, self).__init__()
        self.config = config
        self.net = VQVAE(args=dictToObj(Dict(config)))
    ...
    def forward(self, x):
        ...
        recon_loss, x_recon, vq_output = self.net(x)    
        x_recon = rearrange(x_recon, 'b c t h w -> b t c h w')
        out = [recon_loss, x_recon, vq_output]
        return out
...
if __name__ == "__main__":
    ...
    # import yaml
    # cfg_path = '/mnt/cache/gongjunchao/workdir/radar_forecasting/configs/sevir/ldm_unet.yaml'
    ...
    model = videogpt_vqvae(config=dictToObj(Dict(backbone_kwargs)))
    model.to(device)
    print('end')
    ...
    for i in range(5):
        start.record()
        recon_loss, x_recon, vq_output = model(x=input_data)
...
# srun -p ai4earth --kill-on-bad-exit=1 --quotatype=auto --gres=gpu:1 python -u videogpt_vqvae.py #