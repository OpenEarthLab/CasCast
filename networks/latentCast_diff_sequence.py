...
class _DiT(nn.Module):
    ...
    def forward(self, x, t):
        ...
        """
        B, orig_T, _, _, _ = x.shape
        sequence_length = orig_T // self.split_num

        x = rearrange(x, 'b (s t) c h w -> (b s) (t c) h w', t=sequence_length)
        T = self.split_num
        ...
        out = rearrange(out, 'b (t c) h w -> b t c h w', t=orig_T)  # (N, T, out_channels, H, W)
        return out
...
class latentCast_diff_sequence(nn.Module):
...
if __name__ == "__main__":
    ...
    import yaml
    cfg_path = '/mnt/cache/gongjunchao/workdir/radar_forecasting/configs/sevir/ldm_latentCast_seq12.yaml'
    with open(cfg_path, 'r') as cfg_file:
      cfg_params = yaml.load(cfg_file, Loader = yaml.FullLoader)
    backbone_kwargs = cfg_params['model']['params']['sub_model']['latentCast_diff_sequence']

    # backbone_kwargs = {
    #     'arch': 'DiT-custom',
    #     'config': {
    #     'input_size': h,
    #     'in_channels': 2*c*12//2,
    #     'mlp_ratio': 4.0,
    #     'learn_sigma': False,
    ...
    #     'split_num':2, 'single_heads_num':8
    #     }
    # }
    model = latentCast_diff_sequence(**backbone_kwargs)
    model.to(device)
    print('end')
...
# srun -p ai4earth --kill-on-bad-exit=1 --quotatype=spot --gres=gpu:1 python -u latentCast_diff_sequence.py #