...
class DiT_pred(nn.Module):
    ...
    def forward(self, x, timesteps, cond, context=None, **kwargs):
        ...
        out = rearrange(out, 'b (t c) h w -> b t c h w', t=self.config['pred_length'])
        return out
...
if __name__ == "__main__":
    ...
    import yaml
    cfg_path = '/mnt/cache/gongjunchao/workdir/radar_forecasting/configs/sevir/ldm_latentCast_pred.yaml'
    with open(cfg_path, 'r') as cfg_file:
      cfg_params = yaml.load(cfg_file, Loader = yaml.FullLoader)
    backbone_kwargs = cfg_params['model']['params']['sub_model']['DiT_pred']
    ...
    #     'mlp_ratio': 4.0,
    #     'learn_sigma': False,
    #     'out_channels': pred_length*c,
    #     'pred_length': pred_length,
    #     'depth': 24, 'hidden_size':1024, 'patch_size':2, 'num_heads':16,
    #     }
    # }
    model = DiT_pred(**backbone_kwargs)
    model.to(device)
    print('end')
    ...
    for i in range(5):
        start.record()
        pred = model(x=target, timesteps=t, cond=cond)
    ...
    flops = FlopCountAnalysis(model, (target, t, cond))
    print(flop_count_table(flops))
# srun -p ai4earth --kill-on-bad-exit=1 --quotatype=auto --gres=gpu:1 python -u DiT_pred.py #