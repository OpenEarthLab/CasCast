...
class DiT_framewise(nn.Module):
    ...
    def forward(self, x, timesteps, cond, context=None, **kwargs):
        ...
        b, t, _, h, w = x.shape
        timesteps = timesteps.unsqueeze(dim=1)
        timesteps = timesteps.repeat(1, t)
...
if __name__ == "__main__":
    ...
    backbone_kwargs = {
        ...
        'learn_sigma': False,
        'out_channels': c,
        'depth': 12, 'hidden_size':384, 'patch_size':2, 'num_heads':16,
        }
    }
    model = DiT_framewise(**backbone_kwargs)
    model.to(device)
    print('end')
...
# srun -p ai4earth --kill-on-bad-exit=1 --quotatype=auto --gres=gpu:1 python -u DiT_framewise.py #