...
class MultiscaleBlocks(nn.Module):
    def __init__(self, hidden_size, num_heads, mlp_ratio=4.0, patch_size=[1, 2, 2, 2], **block_kwargs):
        super().__init__()
        ...
        for ps in patch_size:
            if ps == 1:
                ...
                self.multiscale_blocks.append(DiTBlock(hidden_size, num_heads, mlp_ratio=mlp_ratio,
                                                              **block_kwargs))
        
    def forward(self, x, c, x_H, x_W):
        for i, block in enumerate(self.multiscale_blocks):
            x= block(x=x, c=c, x_H=x_H, x_W=x_W, patch_size=self.patch_size[i])
        return x
...
class _DiT(nn.Module):
    ...
    def __init__(
        ...
        super().__init__()
        assert depth == len(block_patch_size) * block_pair_depth
        ...
        self.blocks = nn.ModuleList([
            MultiscaleBlocks(hidden_size, num_heads, mlp_ratio=mlp_ratio, patch_size=block_patch_size) for _ in range(block_pair_depth)
        ])
    ...
    def forward(self, x, t, context=None, y=None):
        ...
        for block in self.blocks:
            x = block(x, c, x_H=x_H, x_W=x_W)                      # (N, T, D)
...
def DiT_L_1(**kwargs):
    return _DiT(hidden_size=1024, patch_size=1, num_heads=16, **kwargs)
...
# srun -p ai4earth --kill-on-bad-exit=1 --quotatype=auto --gres=gpu:1 python -u DiT_window.py #