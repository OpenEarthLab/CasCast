...
class _DiT(nn.Module):
    ...
    def __init__(
        ...
        self.blocks = nn.ModuleList([
            DiTBlock(hidden_size, num_heads, mlp_ratio=mlp_ratio, flash_attention=flash_attention) for _ in range(depth)
        ])
...
if __name__ == "__main__":
    ...
    model = DiT_flash(**backbone_kwargs)
    model.to(device)
    print('end')
...
# srun -p ai4earth --kill-on-bad-exit=1 --quotatype=auto --gres=gpu:1 python -u DiT_flash.py #