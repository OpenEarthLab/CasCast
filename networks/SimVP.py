...
class SimVP(nn.Module):
    def __init__(self, in_shape, hid_S=16, hid_T=256, N_S=4, N_T=4, model_type='gSTA',
                 ...
                 spatio_kernel_dec=3, act_inplace=True, **kwargs):
        super().__init__()
        self.net = SimVP_Model(in_shape=in_shape, hid_S=hid_S, hid_T=hid_T, N_S=N_S, N_T=N_T, 
                               model_type=model_type, mlp_ratio=mlp_ratio, drop=drop, drop_path=drop_path,
                               spatio_kernel_dec=spatio_kernel_dec, spatio_kernel_enc=spatio_kernel_enc,
                               act_inplace=act_inplace, **kwargs)
        self.out_frames = kwargs.get('out_frames', in_shape[1])
    
    def forward(self, x):
        out = self.net(x)
        return out[:, :self.out_frames]
...
if __name__ == "__main__":
    ...
    cfg_path = '/mnt/cache/gongjunchao/workdir/radar_forecasting/configs/sevir/latent_det_prob.yaml'
    with open(cfg_path, 'r') as cfg_file:
      cfg_params = yaml.load(cfg_file, Loader = yaml.FullLoader)
    
    print('end')
    # model = SimVP_Model(cfg_params['model']['params']['sub_model']['EarthFormer'])
    # backbone_kwargs = {
    #     'in_shape': [inp_length, 1, h, w], #T,C,H,W
    #     'hid_S': 32,
    #     'hid_T': 256,
    #     'N_S': 2,
    #     'N_T': 8,
    ...
    #     'act_inplace': True
    # }
    backbone_kwargs = cfg_params['model']['params']['sub_model']['SimVP']
    model = SimVP(**backbone_kwargs)
    model.to(device)
...
# srun -p ai4earth --kill-on-bad-exit=1 --quotatype=auto --gres=gpu:1 python -u SimVP.py # 