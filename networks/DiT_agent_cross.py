...
class DiTCrossBlock(nn.Module):
    ...
    def forward(self, x, c, context, c_h, c_w, h, w):
        ...
        x = x + gate_msa.unsqueeze(1) * self.attn(modulate(self.norm1(x), shift_msa, scale_msa))
        x = x + gate_cross_msa.unsqueeze(1) * self.attn_cross(modulate(self.norm_cross(x), shift_cross_msa, scale_cross_msa), context, H=h, W=w, c_H=c_h, c_W=c_w)
        x = x + gate_mlp.unsqueeze(1) * self.mlp(modulate(self.norm2(x), shift_mlp, scale_mlp))
        return x
...
class _DiT(nn.Module):
    ...
    def __init__(
        ...
        self.blocks = nn.ModuleList([
            DiTCrossBlock(hidden_size, num_heads, context_dim=self.cond_field_hiddensize, mlp_ratio=mlp_ratio, num_patches=num_patches) for _ in range(depth)
        ])
    ...
    def forward(self, x, t, context, y=None):
        ...
        c = t
        for block in self.blocks:
            x = block(x, c, context_hid, c_h=c_h, c_w=c_w, h=h, w=w)                      # (N, T, D)
...
def DiT_B_1(**kwargs):
    return _DiT(depth=12, hidden_size=768, patch_size=1, num_heads=12, **kwargs)
...
class DiT_agent_cross(nn.Module):
...
if __name__ == "__main__":
    ...
    model = DiT_agent_cross(**backbone_kwargs)
    model.to(device)
    print('end')
...
# srun -p ai4earth --kill-on-bad-exit=1 --quotatype=auto --gres=gpu:1 python -u DiT_agent_cross.py #