...
if __name__ == "__main__":
    ...
    from utils.cuboid_transformer_patterns import CuboidSelfAttentionPatterns, CuboidCrossAttentionPatterns
else:
    from .utils.utils import (
        get_activation, get_norm_layer,
        _generalize_padding, _generalize_unpadding,
...
class CuboidTransformerModel(nn.Module):
    ...
    def forward(self, x, verbose=False):
        ...
        # (b, t_out, h, w, c) -> (b, t_out, c, h, w)
        out = rearrange(out, 'b t h w c -> b t c h w')
        return out

class EarthFormer(nn.Module):
    def __init__(self, yaml_model_config, **kwargs):
        super().__init__()
        oc_model_config = OmegaConf.create(yaml_model_config)
        part_model_config = self.get_model_config()
        model_cfg = OmegaConf.merge(oc_model_config, part_model_config)
        ...
        self.net = CuboidTransformerModel(
...
if __name__ == "__main__":
    ...
    import yaml
    from omegaconf import OmegaConf
    cfg_path = '/mnt/cache/gongjunchao/workdir/radar_forecasting/configs/sevir/EarthFormer.yaml'
    with open(cfg_path, 'r') as cfg_file:
      cfg_params = yaml.load(cfg_file, Loader = yaml.FullLoader)
    
    print('end')
    model = EarthFormer(cfg_params['model']['params']['sub_model']['EarthFormer'])
    model.to(device)
...
# srun -p ai4earth --kill-on-bad-exit=1 --quotatype=auto --gres=gpu:1 python -u earthformer.py #