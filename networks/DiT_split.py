...
class MaskLinear(nn.Module):
    ...
    def forward(self, mask: Tensor, input: Tensor) -> Tensor:
        ...
        triple_mask = mask.unsqueeze(0).repeat(3, 1, 1).reshape(-1, mask.shape[1])
        mask_weight = self.weight * triple_mask
        return F.linear(input, mask_weight, self.bias)
...
class SplitAttention(nn.Module):
    def __init__(
        ...
        self.qkv = MaskLinear(dim, dim * 3, bias=qkv_bias)
    ...
    def forward(self, x):
        ...
        B, N, C = x.shape
        qkv = self.qkv(mask=self.split_mask, input=x).reshape(B, N, 3, self.split_num*self.num_heads, self.head_dim).permute(2, 0, 3, 1, 4)
        q, k, v = qkv.unbind(0)
...
if __name__ == "__main__":
    ...
    model = DiT_split(**backbone_kwargs)
    model.to(device)
    print('end')
...
# srun -p ai4earth --kill-on-bad-exit=1 --quotatype=auto --gres=gpu:1 python -u DiT_split.py #