...
class latent_diffusion_cond_model(basemodel):
    ...
    def data_preprocess(self, data):
        data_dict = {}
        inp_data = data['inputs']['coarse'].float().to(self.device, non_blocking=True, dtype=self.data_type)
        era5_field = data['inputs']['field'].float().to(self.device, non_blocking=True, dtype=self.data_type)
        ...
        data_dict.update({'inputs': inp_data, 'era5_field': era5_field,
                          'data_samples': tar_data, 'original': original_tar})
        return data_dict
    
    @torch.no_grad()
    def denoise(self, template_data, cond_data, context_data,
                 bs=1, vis=False, cfg=1, ensemble_member=1):
        """
        denoise from gaussian.
        """
        _, t, c, h, w = template_data.shape
        cond_data = cond_data[:bs, ...]
        context_data = context_data[:bs, ...]
        ...
        if cfg == 1:
            ...
            for t in tqdm(self.sample_noise_scheduler.timesteps) if (self.debug or vis) else self.sample_noise_scheduler.timesteps:
                ...
                noise_pred = self.model[list(self.model.keys())[0]](x=latents, timesteps=timestep, 
                                                                    cond=cond_data, context=context_data)
            ...
            context_data = torch.cat([context_data, torch.zeros_like(context_data)])
            avg_latents = []
            for member in range(ensemble_member):
                ...
                for t in tqdm(self.sample_noise_scheduler.timesteps) if (self.debug or vis) else self.sample_noise_scheduler.timesteps:
                    ...
                    noise_pred = self.model[list(self.model.keys())[0]](x=latent_model_input, timesteps=timestep, cond=cond_data, context=context_data)
                    ########################
    ...
    def train_one_step(self, batch_data, step):
        ...
        era5_field = data_dict['era5_field']
        ...
        if p < self.p_uncond: ## discard condition
            ...
            era5_field_cond = torch.zeros_like(era5_field)
        else:
            z_coarse_prediction_cond = z_coarse_prediction
            era5_field_cond = era5_field
        ## sample noise to add ##
        ...
        if self.visualizer_type is None:
            ...
            z_sample_prediction = self.denoise(template_data=z_tar, context_data=era5_field,
                                               cond_data=z_coarse_prediction, bs=1)
    ...
    def test_one_step(self, batch_data):
        ...
        ## predict the noise residual ##
        noise_pred = self.model[list(self.model.keys())[0]](x=noisy_tar, timesteps=timesteps, cond=z_coarse_prediction, context=era5_field)
    ...
    def eval_step(self, batch_data, step):
        ...
        ## sample image ##
        losses = {}
        z_sample_prediction = self.denoise(template_data=z_tar, context_data=era5_field, cond_data=z_coarse_prediction, bs=tar.shape[0], vis=True, cfg=2, ensemble_member=1)
