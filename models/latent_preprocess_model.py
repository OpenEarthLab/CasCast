...
class latent_preprocess_model(basemodel):
    ...
    def train_one_step(self, batch_data, step):
        ...
        with torch.no_grad():
            if list(self.model.keys())[0] == 'ConvLSTM':
                mask_true = torch.zeros_like(torch.cat([inp, tar], dim=1))
                seq_inp = torch.cat([inp, tar], dim=1)
                coarse_prediction = self.model[list(self.model.keys())[0]](seq_inp, mask_true=mask_true)
                coarse_prediction = coarse_prediction[:, inp.shape[1]-1:]
            else:
            ...
            rec_pred = self.decode_stage(z_coarse_prediction)
            rec_pred = rearrange(rec_pred, '(b t) c h w -> b t c h w', b=b)
            z_coarse_prediction = rearrange(z_coarse_prediction, '(b t) c h w -> b t c h w', b=b)
        prediction_save_names = self.get_save_names(file_names=file_name, size=self.latent_size, model=list(self.model.keys())[0], data_source=self.data_soure)
        self.save_latents(latent_data=z_coarse_prediction, file_names=prediction_save_names)
        loss = self.loss(rec_pred, tar) ## important: rescale the loss
        ## compute csi ##
        if self.metrics_type == 'SEVIRSkillScore':
            data_dict['gt'] = tar
            data_dict['pred'] = rec_pred
            self.eval_metrics.update(target=data_dict['gt'], pred=data_dict['pred'])
        elif self.metrics_type == 'hko7_official':
            data_dict['gt'] = tar
            data_dict['pred'] = rec_pred
            self.eval_metrics.update(gt=data_dict['gt'].squeeze(2).cpu().numpy(), pred=data_dict['pred'].squeeze(2).cpu().numpy(), mask=self.eval_metrics._exclude_mask )
