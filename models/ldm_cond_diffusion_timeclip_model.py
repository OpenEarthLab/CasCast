...
class ldm_cond_diffusion_timeclip_model(basemodel):
    def __init__(self, logger, **params) -> None:
        ...
        ## set frame clip ##
        self.frame_clip = self.params['extra_params']['frame_clip']
        assert self.frame_clip in [1, 2, 3, 4, 6, 12], "frame clip should be in [1, 2, 3, 4, 6, 12] for output length 12"
        self.logger.info(f"################ USING frame_clip: {self.frame_clip} ###################")
    ...
    def denoise(self, template_data, cond_data, context, bs=1, vis=False):
        ...
        for t in tqdm(self.sample_noise_scheduler.timesteps) if (self.debug or vis) else self.sample_noise_scheduler.timesteps:
            ...
            noise_pred = self.model[list(self.model.keys())[1]](x=latents, timesteps=timestep, cond=cond_data, context=context)
    ...
    def train_one_step(self, batch_data, step):
        ...
        ## clip frames ##
        tar = rearrange(tar, 'b (k t) c h w -> (b k) t c h w', t=self.frame_clip)
        b, t, c, h, w = tar.shape
        ## warning: using right order ##
        field_data = torch.cat([field_data]*(12//self.frame_clip), dim=1)
        field_data = rearrange(field_data, 'b (k t) c h w -> (b k) t c h w', k=(12//self.frame_clip)).contiguous()
        ...
        if self.visualizer_type is None:
            ...
            ## TODO: inverse the clip ##
            denoise_bs = 1 * 12//self.frame_clip
            z_sample_prediction = self.denoise(template_data=z_tar, cond_data=z_coarse_prediction, context=field_data, bs=denoise_bs)
            ...
            sample_prediction = rearrange(sample_prediction, '(b t) c h w -> b t c h w', t=12) 
            self.visualizer.save_pixel_image(pred_image=sample_prediction, target_img=rearrange(tar, '(b k) t c h w -> b (k t) c h w', k=12//self.frame_clip), step=step)
        else:
            pass
