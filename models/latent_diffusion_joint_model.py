...
class latent_diffusion_joint_model(basemodel):
    ...
    def train_one_step(self, batch_data, step):
        ...
        b, t, c, h, w = tar.shape
        ## first train the deterministic predictor in the latent space ##
        ## model 2 is the deterministic model ##
        coarse_prediction = self.model[list(self.model.keys())[2]](inp)
        deterministic_loss = self.loss(coarse_prediction, tar)
        ## inp is coarse prediction in latent space, tar is gt in latent space
        z_tar = tar
        z_coarse_prediction = coarse_prediction
        ...
        diffusion_loss = self.loss(noise_pred, noise) ## important: rescale the loss
        loss = diffusion_loss + deterministic_loss
        loss.backward()

        ## update params of deterministic model ##
        ...
        if self.visualizer_type is None:
            ...
            pass
        return {self.loss_type: loss.item(), 'deterministic_loss': deterministic_loss.item(), 'diffusion_loss': diffusion_loss.item()}
    ...
    def test_one_step(self, batch_data):
        ...
        ## inp is coarse prediction in latent space, tar is gt in latent space
        z_tar = tar
        z_coarse_prediction = self.model[list(self.model.keys())[2]](inp)
        ## scale ##
        z_tar = z_tar * self.scale_factor
        z_coarse_prediction = z_coarse_prediction * self.scale_factor
        ## sample noise to add ##
        ...
        diffusion_MSE_loss = torch.mean((noise_pred - noise) ** 2).item()
        deterministic_MSE_loss = torch.mean((z_coarse_prediction - z_tar) ** 2).item()
        loss =  diffusion_MSE_loss + deterministic_MSE_loss
        ...
        if self.metrics_type == 'hko7_official':
            ...
            loss_records.update({'MSE': loss, 'diffusion_loss': diffusion_MSE_loss, 'deterministic_loss': deterministic_MSE_loss})
    ...
    def eval_step(self, batch_data, step):
        ...
        elif self.visualizer_type == 'hko7_visualizer' and (step) % 20 == 0:
            self.visualizer.save_pixel_image(pred_image=data_dict['pred'], target_img=data_dict['gt'], step=step)
        else:
            pass
    ...
    def test_final(self, test_data_loader, predict_length):
        ...
        if self.metrics_type == 'SEVIRSkillScore':
            ...
            self.scale_factor = 0.4666207432746887 #60x60x4
        else:
            raise NotImplementedError
        # set model to eval
        for key in self.model:
