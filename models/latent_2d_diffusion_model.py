...
class latent_2d_diffusion_model(basemodel):
    ...
    def denoise(self, template_data, cond_data, bs=1, vis=False, cfg=1, ensemble_member=1):
        ...
        # latents = torch.randn(
        #     (bs*ensemble_member, 1, c, h, w),
        #     generator=generator,
        #     device=template_data.device,
        # ) 
        # latents = latents.expand(bs*ensemble_member, t, c, h, w)
    ...
    def train_one_step(self, batch_data, step):
        ...
        ## sample noise to add ##
        # noise = torch.randn((b, 1, c, h, w), device=inp.device, dtype=inp.dtype)
        # noise = noise.expand(b, t, c, h, w)
        noise = torch.randn((b, t, c, h, w), device=inp.device, dtype=inp.dtype)
