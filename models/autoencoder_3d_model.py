...
class autoencoder_3d_model(basemodel):
    ...
    def data_preprocess(self, data):
        ...
        total_data = torch.cat([inp_data, tar_data], dim=1)
        start_idx = np.random.randint(0, inp_data.shape[1]+1)
        inp = total_data[:, start_idx:start_idx+tar_data.shape[1], ...]
        tar = inp
        data_dict.update({'inputs': inp, 'data_samples': tar})
        return data_dict

    def get_last_layer(self):
        if utils.get_world_size() == 1 :
            last_layer = self.model[list(self.model.keys())[0]].get_last_layer_weight()
        else:
            last_layer = self.model[list(self.model.keys())[0]].module.get_last_layer_weight()
        return last_layer
    ...
    def train_one_step(self, batch_data, step):
        ...
        rec_frames = self.video2frames(reconstruction)
        tar_frames = self.video2frames(tar)
        aeloss, log_dict_ae = self.model[list(self.model.keys())[1]](inputs=tar_frames, reconstructions=rec_frames, posteriors=posterior, 
        ...
        disloss, log_dict_disc = self.model[list(self.model.keys())[1]](tar_frames, rec_frames, posterior, optimizer_idx=1, global_step=step,
        ...
        if self.visualizer_type is None:
            ...
            self.visualizer.save_pixel_image(pred_image=reconstruction, target_img=tar, step=step)
        else:
            pass
    ...
    def test_one_step(self, batch_data):
       ...
       ## first: train encoder+decoder+logvar ##
        ...
        ## second: train the discriminator ##
        disloss, log_dict_disc = self.model[list(self.model.keys())[1]](tar_frames, rec_frames, posterior, optimizer_idx=1, global_step=0,
