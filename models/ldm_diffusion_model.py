...
class ldm_diffusion_model(basemodel):
    ...
    def eval_step(self, batch_data, step):
        ...
        ## sample image ##
        losses = {}
        z_sample_prediction = self.denoise(template_data=z_tar, cond_data=z_coarse_prediction, bs=tar.shape[0], vis=True, cfg=2, ensemble_member=5)
    ...
    def test_final(self, test_data_loader, predict_length):
        ...
        metric_logger = utils.MetricLogger(delimiter="  ", sync=True)
        self.scale_factor = 0.6790619492530823
        # set model to eval
        for key in self.model:
            self.model[key].eval()
