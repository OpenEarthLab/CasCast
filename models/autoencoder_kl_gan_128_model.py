...
class autoencoder_kl_gan_128_model(basemodel):
    ...
    def data_preprocess(self, data):
        data_dict = {}
        inp_data = F.interpolate(data['inputs'].float().to(self.device, non_blocking=True, dtype=self.data_type), size=(128, 128))
        tar_data = F.interpolate(data['data_samples'].float().to(self.device, non_blocking=True, dtype=self.data_type), size=(128, 128))
