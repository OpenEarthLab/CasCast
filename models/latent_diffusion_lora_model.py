...
class latent_diffusion_lora_model(basemodel):
    def __init__(self, logger, **params) -> None:
        ...
        self.base_diffusion_model_ckpt_path = self.extra_params.get("base_diffusion_model_path", None)
        print(f'load from autoencoder_ckpt_path: {self.autoencoder_ckpt_path}')
        self.load_checkpoint(self.base_diffusion_model_ckpt_path, load_model=True, load_optimizer=False, load_scheduler=False, load_epoch=False, load_metric_best=False, 
                             lora=True, lora_base_model='DiT')
    ...
    def data_preprocess(self, data):
        ...
        original_event_type = data['data_samples']['event_type']
        # original_event_type = torch.randint(0, 6, (inp_data.shape[0],)) ## TODO
        data_dict.update({'inputs': inp_data, 'data_samples': tar_data, 'original': original_tar, 'event_type': original_event_type})
        return data_dict
    
    @torch.no_grad()
    def denoise(self, template_data, cond_data, y, bs=1, vis=False, cfg=1, ensemble_member=1):
        """
        ...
        if cfg == 1:
            ...
            for t in tqdm(self.sample_noise_scheduler.timesteps) if (self.debug or vis) else self.sample_noise_scheduler.timesteps:
                ...
                noise_pred = self.model[list(self.model.keys())[0]](x=latents, timesteps=timestep, cond=cond_data, y=y)
            ...
            for member in range(ensemble_member):
                ...
                for t in tqdm(self.sample_noise_scheduler.timesteps) if (self.debug or vis) else self.sample_noise_scheduler.timesteps:
                    ...
                    noise_pred = self.model[list(self.model.keys())[0]](x=latent_model_input, timesteps=timestep, cond=cond_data, y=y)
                    ########################
    ...
    def train_one_step(self, batch_data, step):
        ...
        original_event_type = data_dict['event_type']
        ...
        if self.visualizer_type is None:
            ...
            z_sample_prediction = self.denoise(template_data=z_tar, cond_data=z_coarse_prediction, y=original_event_type, bs=1)
    ...
    def eval_step(self, batch_data, step):
        ...
        ### use different lora ###
        for event_type in range(9):
            original_event_type[0] = event_type
            z_sample_prediction = self.denoise(template_data=z_tar, cond_data=z_coarse_prediction, bs=tar.shape[0], vis=True, cfg=2, ensemble_member=1,
                                        y=original_event_type)
            len_shape_prediction = len(z_sample_prediction.shape)
            assert len_shape_prediction == 6
            ...
            if self.visualizer_type == 'sevir_visualizer' and (step) % 1 == 0:
                self.visualizer.save_pixel_image(pred_image=data_dict['pred'], target_img=data_dict['gt'], step=step+event_type)
            else:
                pass
