...
class mcvd_diffusion_staticNorm_model(basemodel):
    def __init__(self, logger, **params) -> None:
        ...
        self.static_mean = 18.0574
        self.static_std = 26.4573
    ...
    def denoise(self, template_data, cond_data, bs=1):
        ...
        np.save('staticNormDenoise.npy', latents_list.cpu().numpy())
        return latents
    ...
    def train_one_step(self, batch_data, step):
        ...
        scale_coarse_prediction = (coarse_prediction * 255.0 - self.static_mean) / self.static_std
        scale_tar = (tar * 255.0 - self.static_mean) / self.static_std
        ...
        if self.visualizer_type is None:
            ...
            sample_prediction = (scale_sample_prediction * self.static_std + self.static_mean) / 255.0
    ...
    def eval_step(self, batch_data, step):
        ...
        ## first predict coarse prediction ##
        coarse_prediction = self.model[list(self.model.keys())[0]](inp)
        scale_coarse_prediction = coarse_prediction * self.noise_scale
        scale_tar = tar * self.noise_scale
        ## noise prediction ##
        ## sample noise to add ##
        noise = torch.randn_like(tar)
        ## sample random timestep for each ##
        ...
        refined_pred = scale_refined_pred / self.noise_scale
        ## evaluate other metrics ##
        data_dict = {}
