...
class latent_diffusion_model(basemodel):
    ...
    def eval_step(self, batch_data, step):
        ...
        if self.visualizer_type == 'sevir_visualizer' and (step) % 1 == 0:
            ...
            # ceph_prefix = f'radar:s3://radar_visualization/sevir/cascast_{model_name}_{self.visualizer.sub_dir}'
            ...
            #######################################################################################################
            model_name = list(self.model.keys())[0]
            ceph_prefix = f'radar:s3://radar_visualization/hko7/cascast_{model_name}_{self.visualizer.sub_dir}'
            ...
            # self.visualizer.save_pixel_image(pred_image=data_dict['pred'], target_img=data_dict['gt'], step=step)
            model_name = list(self.model.keys())[0]
            ceph_prefix = f'radar:s3://radar_visualization/meteonet/cascast_{model_name}_{self.visualizer.sub_dir}'
    ...
    def test_final(self, test_data_loader, predict_length):
        ...
        if self.metrics_type == 'SEVIRSkillScore':
            ...
            self.scale_factor = 0.602343738079071 #50x50x4
        else:
            raise NotImplementedError
        # set model to eval
        for key in self.model:
