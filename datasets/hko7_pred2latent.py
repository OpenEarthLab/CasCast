...
class hko7_pred2latent(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        packed_results = dict()
        packed_results['inputs'] = frame_data[:self.input_length].unsqueeze(1)
        packed_results['data_samples'] = frame_data[self.input_length:].unsqueeze(1)
        packed_results['file_name'] = self.init_times[idx]
...
if __name__ == '__main__':
    ...
    dataset = hko7_pred2latent(split='test', input_length=10, pred_length=10, base_freq=base_freq)
...
### srun -p ai4earth --kill-on-bad-exit=1 --quotatype=reserved --gres=gpu:0 python -u hko7_pred2latent.py ###
