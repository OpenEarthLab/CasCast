...
def get_sevir_latent25_and_coarse_pred_dataset( split, input_length=13, pred_length=12, data_dir='/mnt/data/oss_beijing/video_prediction_dataset/sevir/sevir', base_freq='5min', height=384, width=384, **kwargs):
    if data_dir == '/mnt/data/oss_beijing/video_prediction_dataset/sevir/sevir':
        ...
        return sevir_latent25_and_coarse_pred_sproject(split, input_length=input_length, pred_length=pred_length, data_dir=data_dir, base_freq=base_freq, height=height, width=width, **kwargs)
    else:
        raise NotImplementedError



class sevir_latent25_and_coarse_pred_sproject(Dataset):
    def __init__(self, split, input_length=13, pred_length=12, data_dir='radar:s3://sevir_latent', base_freq='5min', height=384, width=384, 
        ...
        self.compress_data_source = kwargs.get('compress_data_source', 'sevir_25')
        self.coarse_model = kwargs.get('coarse_model', 'earthformer')
    ...
    def __getitem__(self, index):
        ...
        gt_latent_data = self._load_latent_frames(file, datasource=self.compress_data_source)
        coarse_latent_data = self._load_latent_frames(file, datasource=self.coarse_model)
        packed_results = dict()
        packed_results['inputs'] = {'coarse_pred':coarse_latent_data, 'prev_inp':gt_latent_data[:self.input_length]}
        packed_results['data_samples'] = {'latent':gt_latent_data[self.input_length:], 'original':gt_data}
        return packed_results

if __name__ == "__main__":
    dataset = get_sevir_latent25_and_coarse_pred_dataset(split='train', input_length=13, pred_length=12,
                                                         compress_data_source='sevir_25', coarse_model='earthformer',
                                                          data_dir='radar:s3://sevir_latent', base_freq='5min', height=384, width=384)
    print(len(dataset))
...
### srun -p ai4earth --kill-on-bad-exit=1 --quotatype=reserved --gres=gpu:0 python -u sevir_latent25.py ###